...
use config::Config;
use commands::{CommandRegistry, CommandHandler, PingCommand, HelpCommand, UptimeCommand, EightBallCommand};
use twitch::{TwitchClient, OAuthManager};
...
async fn start_bot(_debug: bool, prefix: String, channel_override: Option<String>) -> Result<()> {
    ...
    // Join channel
    client.join_channel(&config.channel_name, &config.bot_username).await?;
    ...
    let descriptions = vec![
        ...
        ("uptime".to_string(), "Shows how long the bot has been running".to_string()),
        ("help".to_string(), "Shows help information for available commands".to_string()),
        ("8ball".to_string(), "Ask the Magic 8-Ball a yes/no question. Usage: !8ball <question>".to_string()),
    ];
